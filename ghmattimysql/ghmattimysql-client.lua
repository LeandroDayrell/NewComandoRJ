...
function setNuiActive(booleanOrNil)
	...
	if booleanOrNil ~= nil then
		boolean = booleanOrNil
	end
...
AddEventHandler("ghmattimysql:update-resource-data",function(resourceData)
	...
	for resource,data in pairs(resourceData) do
		table.insert(arrayToSortAndMap,{ resource = resource, queryTime = data.totalExecutionTime, count = data.queryCount })
	end
	...
	if #arrayToSortAndMap > 0 then
		...
		SendNUIMessage({ type = "onResourceLabels", resourceLabels = map(arrayToSortAndMap,function(el) return el.resource end) })
		SendNUIMessage({ type = "onResourceData",
			resourceData = {
...
AddEventHandler("ghmattimysql:update-time-data",function(timeData)
	...
	if #timeArray > 0 then
		SendNUIMessage({ type = "onTimeData",
			timeData = {
...
AddEventHandler("ghmattimysql:update-slow-queries",function(slowQueryData)
	...
	SendNUIMessage({ type = "onSlowQueryData", slowQueries = slowQueries })
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- UPDATE-STATUS
-----------------------------------------------------------------------------------------------------------------------------------------
...
AddEventHandler("ghmattimysql:update-status",function(statusData)
	SendNUIMessage({ type = "onStatusData", status = statusData })
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- UPDATE-VARIABLES
-----------------------------------------------------------------------------------------------------------------------------------------
...
AddEventHandler("ghmattimysql:update-variables",function(variableData)
	SendNUIMessage({ type = "onVariableData", variables = variableData })
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- SERVER-STATUS
-----------------------------------------------------------------------------------------------------------------------------------------
...
AddEventHandler("ghmattimysql:get-table-schema",function(tableName,schema)
	SendNUIMessage({ type = "onTableSchema", tableName = tableName, schema = schema })
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- REQUEST-TABLE
-----------------------------------------------------------------------------------------------------------------------------------------
