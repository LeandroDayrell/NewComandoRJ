	...
	$.post("http://vrp_crafting/requestCrafting",JSON.stringify({ craft: selectCraft }),(data) => {
		...
		for (let x=1; x <= inSlots; x++){
			...
			if (nameList2[x-1] !== undefined){
				...
				const item = `<div class="item2 populated" style="background-image: url('nui://vrp_inventory/web-side/images/${v.index}.png');" data-item-key="${v.key}" data-name-key="${v.name}" data-item-list="${list}" data-slot="${slot}">
