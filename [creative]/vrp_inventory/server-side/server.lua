...
-- ITEMPICKUP
-----------------------------------------------------------------------------------------------------------------------------------------
function cRP.itemPickup(grid,id)
	local source = source
	local user_id = vRP.getUserId(source)
	if user_id then
			...
			if inv then
				...
				if vRP.itemTypeList(itemName) == "use" then
					...
					if itemName == "weedendolada" then
						...
						repeat
							if active[user_id] == 0 then
								...
								if vRP.getInventoryItemAmount(user_id,"weedendolada") >= parseInt(rAmount) and vRP.getInventoryItemAmount(user_id,"silk") >= parseInt(rAmount) then
									if vRP.tryGetInventoryItem(user_id,"weedendolada",parseInt(rAmount),true,slot) and vRP.tryGetInventoryItem(user_id,"silk",parseInt(rAmount),true) then
								...
								if taskResult then
									...
									if 100 >= 70 then
										...
										for k,v in pairs(copAmount) do
											async(function()
												TriggerClientEvent("NotifyPush",v,{ code = 31, title = "Roubo de Veículo", x = x, y = y, z = z, vehicle = vRP.vehicleName(vehName).." - "..vehPlate, rgba = {15,110,110} })
											end)
										end
									end
								end
									...
									repeat
										if active[user_id] == 0 then
											...
											if 100 >= 100 then
												...
												for k,v in pairs(copAmount) do
													async(function()
														TriggerClientEvent("NotifyPush",v,{ code = 20, title = "Roubo a Caixa Registradora", x = x, y = y, z = z, rgba = {170,80,25} })
													end)
												end
											end
										end
					...
					if itemName == "rope" then
						local nplayer = vRPclient.nearestPlayer(source,2)
						if nplayer and (vRPclient.getHealth(nplayer) <= 101 or vPLAYER.getHandcuff(nplayer)) then
