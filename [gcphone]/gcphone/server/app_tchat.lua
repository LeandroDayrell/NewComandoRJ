function TchatGetMessageChannel(channel,cb)
	MySQL.Async.fetchAll("SELECT * FROM phone_chat WHERE channel = @channel ORDER BY time DESC LIMIT 100",{ ['@channel'] = channel },cb)
end

function TchatAddMessage(channel,message)
	local Query = "INSERT INTO phone_chat (`channel`,`message`) VALUES(@channel,@message);"
	local Query2 = "SELECT * from phone_chat WHERE `id` = @id;"
