...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustPressed(1,311) and not IsEntityInWater(ped) and vSERVER.checkPhone() then
			vRP.stopActived()
...
AddEventHandler("gcPhone:activePhone",function()
	if phoneActive then
		phoneActive = false
		TriggerEvent("Notify","importante","Desativado o modo avião.",3000)
	else
		phoneActive = true
		TriggerEvent("Notify","importante","Ativado o modo avião.",3000)
	end
end)
...
AddEventHandler("gcPhone:waitingCall",function(infoCall,initiator)
	...
	if not initiator and phoneActive then
		TriggerEvent("Notify","sucesso","Chamada recebida.",3000)
	end
...
RegisterNUICallback("sendMessage",function(data,cb)
	if data.message == '%pos%' then
		...
		data.message = 'Location: '..mathLegth(myPos.x)..', '..mathLegth(myPos.y)
	end
