...
AddEventHandler('vehicleshop.sussessbuy', function(vehicleName,value)    
    SendNUIMessage(
        {
            type = "sussess-buy",
...
RegisterNUICallback(
    "Buy",
    function(data, cb)
        TriggerServerEvent('vehicleshop.CheckMoneyForVeh',data.modelcar, data.sale, "car", data.name, {rgbColorSelected, rgbSecondaryColorSelected})
 
    end
)

RegisterNetEvent('vehicleshop.spawnVehicle')
AddEventHandler('vehicleshop.spawnVehicle', function(model)    
    local hash = GetHashKey(model)
 ...
 --   SetVehicleCustomPrimaryColour(vehicleBuy,  rgbColorSelected[1], rgbColorSelected[2], rgbColorSelected[3])
  --  SetVehicleCustomSecondaryColour(vehicleBuy,  rgbSecondaryColorSelected[1], rgbSecondaryColorSelected[2], rgbSecondaryColorSelected[3])
end)
